...
pub struct PodX3Module;

impl Module for PodX3Module {
    fn config(&self) -> Box<[Config]> {
        vec![
            config::PODX3_LIVE_CONFIG.clone(),
        ].into_boxed_slice()
    }

    fn init(&self, config: &'static Config) -> Box<dyn Interface> {
        Box::new(PodX3Interface::new(config))
    }
...
impl Interface for PodX3Interface {
    ...
    fn wire(&self, edit: Arc<Mutex<EditBuffer>>, callbacks: &mut Callbacks) -> anyhow::Result<()> {
        ...
        // TODO resolve_footswitch_mode_show(&self.objects, config)?;
    ...
    fn init(&self, edit: Arc<Mutex<EditBuffer>>) -> anyhow::Result<()> {
         ...
         */
        controller.set_full("loop_enable:show", 1, MIDI, Signal::Force);
